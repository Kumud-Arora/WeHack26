...
@app.route("/sample")
def sample():
    sample_files = sorted(OUTPUT_DIR.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not sample_files:
        flash("No sample results available yet.", "error")
        return redirect(url_for("index"))
    json_path = sample_files[0]
    with open(json_path, encoding="utf-8") as fp:
        data = json.load(fp)
    category_totals = data.get("derived_metrics", {}).get("category_totals", {})
    max_cat_value = max(category_totals.values(), default=1)
    return render_template("results.html", data=data, result_id=json_path.stem,
                           category_totals=category_totals, max_cat_value=max_cat_value)


@app.route("/api/memory")
def api_memory():
    memory_path = Path("call_analyzer/data/memory.json")
    if not memory_path.exists():
        return jsonify({})
    with open(memory_path, encoding="utf-8") as fp:
        memory = json.load(fp)
    return jsonify(memory.get("demo_user", {}))
